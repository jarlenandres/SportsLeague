...
internal class PlayerService : IPlayerService
{
    ...
    public async Task<Player> CreateAsync(Player player)
    {
        ...
        if (!teamExists)
        {
            ...
            throw new KeyNotFoundException($"No se encontro el equipo con ID {player.TeamId}");
        }

        // Validar que el número de camisa es único dentro del equipo
        var existingPlayer = await _playerRepository.GetByTeamAndNumberAsync(player.TeamId, player.Number);
        if (existingPlayer != null)
        {
            _logger.LogWarning("Number {Number} already taken in team {TeamId}.", player.Number, player.TeamId);
            throw new InvalidOperationException($"El número {player.Number} ya está en uso");
        }

        _logger.LogInformation("Creating player: {FirstName} {LastName}", player.FirstName, player.LastName);
    ...
    public async Task UpdateAsync(int id, Player player)
    {
        ...
        // Validar que el número de camisa es único dentro del equipo (si cambio el número o el equipo)
        if (existingPlayer.Number != player.Number || existingPlayer.TeamId != player.TeamId)
        {
            ...
            {
                throw new InvalidOperationException($"El número {player.Number} ya está en uso en el equipo");
            }
        }
